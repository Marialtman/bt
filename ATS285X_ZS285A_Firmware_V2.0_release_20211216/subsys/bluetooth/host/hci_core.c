...
static void bt_sco_conn_req(struct bt_hci_evt_conn_request *evt)
{
	...
	if (!bt_conn_notify_connect_req(&evt->bdaddr, BT_CONN_TYPE_SCO)) {
...
static void conn_req(struct net_buf *buf)
{
	...
	if (conn) {
		/* Already connected, avoid repeat connect request attack */
		bt_conn_unref(conn);
		return;
	}

	if (!bt_conn_notify_connect_req(&evt->bdaddr, BT_CONN_TYPE_BR)) {
		reject_conn(&evt->bdaddr, BT_HCI_ERR_LOCAL_USER_TERM_CONN);
		return;
	}
...
#ifdef CONFIG_BR_ENABLE_SSP
	/* Controler not support set disable, not set, controler default value is disable. */
