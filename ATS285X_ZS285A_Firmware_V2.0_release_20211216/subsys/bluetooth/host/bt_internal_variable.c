...
BT_DATA_POOL_DEFINE(host_rx_pool, 1, CONFIG_BT_DATA_POOL_RX_SIZE, &continue_data_pool_opt);
BT_DATA_POOL_DEFINE(host_tx_pool, 1, CONFIG_BT_DATA_POOL_TX_SIZE, &continue_data_pool_opt);
...
int btstack_property_set(const char *key, char *value, int value_len)
{
	int ret = -EIO;
...
int btstack_property_get(const char *key, char *value, int value_len)
{
	int ret = -EIO;
