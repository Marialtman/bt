...
int hostif_stack_storage_get_linkkey(struct br_linkkey_info *info, u8_t cnt)
{
	...
	prio = hostif_set_negative_prio();
	ret = stack_storage_get_linkkey(info, cnt);
	hostif_revert_prio(prio);
...
int hostif_stack_storage_update_linkkey(struct br_linkkey_info *info, u8_t cnt)
{
	...
	prio = hostif_set_negative_prio();
	ret = stack_storage_update_linkkey(info, cnt);
	hostif_revert_prio(prio);
...
int hostif_stack_storage_write_ori_linkkey(bt_addr_t *addr, u8_t *link_key)
{
	...
	prio = hostif_set_negative_prio();
	ret = stack_storage_write_ori_linkkey(addr, link_key);
	hostif_revert_prio(prio);
...
void hostif_bt_l2cap_reg_br_conn_rsp_cb(bt_br_conn_rsp_result_cb cb)
{
	...
	prio = hostif_set_negative_prio();
	bt_l2cap_reg_br_conn_rsp_cb(cb);
	hostif_revert_prio(prio);
#endif
}
