...
extern int ok_mic_input_event_proc(struct app_msg *msg);
#endif
...
void system_key_event_handle(struct app_msg *msg)
{
	...
	#endif
#ifdef CONFIG_OK_MIC_APP
		if (ok_mic_input_event_proc(msg))
			SYS_LOG_ERR("error: 0x%x!\n", msg->cmd);
	#endif
		break;
	}
...
void system_hotplug_event_handle(struct app_msg *msg)
{
	...
	case HOTPLUG_USB_HOST:
		if (msg->value == HOTPLUG_IN) {
	#ifdef CONFIG_OTA_BACKEND_SDCARD //tcf 2021/9/1 10:23
		if(!get_reboot_from_ota_flag())
		{
			if(ota_app_init_usb() == 0)
				break;
		}
	#endif
