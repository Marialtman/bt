...
#include <ota/ota_backend_uart.h>
#include <ota_app.h>
...
#if	defined(CONFIG_OTA_BACKEND_SDCARD) || defined(CONFIG_OTA_BACKEND_UHOST)
...
{
	backend_sdcard = ota_backend_sdcard_init(ota_app_backend_callback, &uhost_init_param);
	if (!backend_sdcard) {
