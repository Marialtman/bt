...
static int _aset_write_case_info(struct ASET_data *data)
{
	...
	aset_case_info->max_eq_point_num = ASET_DAE_PEQ_POINT_NUM;
	aset_case_info->bMax_Volume = audio_policy_get_volume_level();


	_aset_config_pc_view(&aset_case_info->stInterface);

	print_buffer(aset_case_info, 1, sizeof(aset_case_info_t), 16, -1);
...
static int _aset_volume_table_deal(struct ASET_data *data)
{
	...
	if (_aset_stub_read(data, STUB_CMD_ASET_READ_VOLUME_TABLE_DATA_V3,
