...
static u32_t soc_cmu_light_sleep(void)
{
	...
	/* HOSC clock select OP smiths trigger output and increase HGMC */
	sys_write32((sys_read32(CMU_HOSC_CTL) & ~0xffff) | 0x7341, CMU_HOSC_CTL);
...
static void soc_cmu_ligth_wakeup(u32_t corepll_backup)
{
	...
	/* HOSC clock select slicer output and decrease HGMC */
	sys_write32((sys_read32(CMU_HOSC_CTL) & ~0xffff) | 0x7051, CMU_HOSC_CTL);
