...
media_player_t *media_player_open(media_init_param_t *init_param)
{
...
#ifdef CONFIG_OK_MIC_APP
	if (okmic_ctx->player && init_param->stream_type != AUDIO_STREAM_OKMIC) {
		init_param->attach_stream_type = init_param->stream_type;
		if ( init_param->stream_type == AUDIO_STREAM_LINEIN) {

			if (okmic_ctx->player) {
				okmic_player_stop_play();
			}
			okmic_ctx->stream_type = AUDIO_STREAM_OKMIC_LINE_IN;
			init_param->stream_type = AUDIO_STREAM_OKMIC_LINE_IN;
			init_param->capture_channels = 2;
			init_param->channels = 2;
		} else if (init_param->stream_type == AUDIO_STREAM_FM) {
			if (okmic_ctx->player) {
				okmic_player_stop_play();
			}

			okmic_ctx->stream_type = AUDIO_STREAM_OKMIC_FM;
			init_param->stream_type = AUDIO_STREAM_OKMIC_FM;
			init_param->capture_channels = 2;
			init_param->channels = 2;
		} else {
			init_param->output_stream = okmic_player_create_attached_stream(init_param->stream_type);
		}
	}
	...
	/**aac only support Direct forwarding mode*/
	if (srv_param.tws_observer && (bt_manager_tws_get_dev_role() == BTSRV_TWS_SLAVE
		|| (init_param->stream_type == AUDIO_STREAM_MUSIC
		&& init_param->format == AAC_TYPE)
		|| (srv_param.tws_mode == MEDIA_TWS_MODE_STEREO
			&& init_param->format == SBC_TYPE))) {
		btif_tws_set_input_stream(init_param->input_stream);
		handle->need_set_stream = 1;
	}
	...
	if (is_tws) {
		...
		media_player_set_output_mode(handle, MEDIA_OUTPUT_MODE_L_R_MIX);
...
#ifdef CONFIG_OK_MIC_APP
	if (init_param->stream_type != AUDIO_STREAM_OKMIC) {
		if (init_param->stream_type == AUDIO_STREAM_OKMIC_FM ||
			init_param->stream_type == AUDIO_STREAM_OKMIC_LINE_IN) {
			okmic_ctx->player = handle;
		} else {
			okmic_player_restart(init_param->output_stream, init_param->capture_output_stream, init_param->channels, init_param->capture_format, init_param->sample_rate, init_param->attach_stream_type);
		}
	} else {
		handle->is_okmic = 1;
	}
...
error_exit:
	...
	if (init_param->output_stream) {
		stream_destroy(init_param->output_stream);
...
int media_player_close(media_player_t *handle)
{
...
#ifdef CONFIG_TWS
	if (handle->need_set_stream) {
		btif_tws_set_input_stream(NULL);
	}
#endif
	...
	//if support okmic ,need reset okmic to default
#ifdef CONFIG_OK_MIC_APP
	if (handle->is_okmic != 1) {
		media_player_okmic_ctx_s  *okmic_ctx= get_okmic_player_ctx();
		if (okmic_ctx->stream_type == AUDIO_STREAM_OKMIC_FM ||
			okmic_ctx->stream_type == AUDIO_STREAM_OKMIC_LINE_IN) {
			okmic_player_start_play();
		} else {
			okmic_player_restart(NULL, NULL, 0, 0, 16, 0);
		}
	}
#endif
...
int media_player_switch_effect_param(media_player_t *handle, const u8_t *efx_name)
{
	...
	return media_player_update_effect_param(handle, (void *)vaddr, 512);
}
