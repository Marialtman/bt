...
void ota_breakpoint_dump(struct ota_breakpoint *bp)
{
	...
	SYS_LOG_INF("  old_version 0x%x, new_version 0x%x",
		bp->old_version, bp->new_version);
	...
	SYS_LOG_INF("  cur file %s: file_id %d, offset 0x%x, size 0x%x",
		cur_file->name, cur_file->file_id, cur_file->offset, cur_file->size);
	SYS_LOG_INF("  write_offset 0x%x\n",
		...
		int state, int cur_offset, int force)
{
	...
	SYS_LOG_DBG("update bp file_id %d state %d offs 0x%x", file->file_id, state, cur_offset);
