...
#define TLV_PACK_U8(buf, type, value)	tlv_pack_data(buf, type, sizeof(u8_t), value)
#define TLV_PACK_U16(buf, type, value)	tlv_pack_data(buf, type, sizeof(u16_t), value)
#define TLV_PACK_U32(buf, type, value)	tlv_pack_data(buf, type, sizeof(u32_t), value)

#define TLV_UNPACK_U8(ctx, type, value_ptr)	\
		tlv_unpack_data(ctx, type, NULL, (u8_t *)value_ptr, sizeof(u8_t))
#define TLV_UNPACK_U16(ctx, type, value_ptr)	\
		tlv_unpack_data(ctx, type, NULL, (u8_t *)value_ptr, sizeof(u16_t))
#define TLV_UNPACK_U32(ctx, type, value_ptr)	\
		tlv_unpack_data(ctx, type, NULL, (u8_t *)value_ptr, sizeof(u32_t))
...
typedef int (*svc_prot_cmd_handler_t)(struct svc_prot_context *ctx, u16_t param_len);
struct svc_prot_cmd {
	u8_t cmd;
...
static int svc_prot_get_rx_data(struct svc_prot_context *ctx, u8_t *buf, int size)
{
	int read_size;
...
static int svc_prot_skip_rx_data(struct svc_prot_context *ctx, int size)
{
	...
	while (size > 0) {
		err = svc_prot_get_rx_data(ctx, &c, sizeof(u8_t));
		if (err) {
...
static u8_t *tlv_pack(u8_t *buf, const struct tlv_data *tlv)
{
	u16_t len;
...
static u8_t *tlv_pack_data(u8_t *buf, u8_t type, u16_t len, u32_t number)
{
	struct tlv_data tlv;

	tlv.type = type;
	tlv.len = len;
	tlv.value_ptr = (u8_t *)&number;
...
static int tlv_unpack_head(struct svc_prot_context *ctx, struct tlv_data *tlv)
{
	...
	/* read length */
	err = svc_prot_get_rx_data(ctx, (u8_t *)&tlv->len, sizeof(tlv->len));
	if (err) {
		SYS_LOG_ERR("failed to read tlv type");
		return err;
	}
...
static int tlv_unpack_data(struct svc_prot_context *ctx, u8_t type, int *len,
...
int svc_prot_send_data(struct svc_prot_context *ctx, u8_t *buf, int size)
{
	int err;
...
int ota_bt_send_cmd(struct svc_prot_context *ctx, u8_t cmd,
		    u8_t *buf, int size)
{
...
int ota_cmd_h2d_request_upgrade(struct svc_prot_context *ctx, u16_t param_size)
{
...
int ota_cmd_h2d_connect_negotiation(struct svc_prot_context *ctx, u16_t param_len)
{
...
int ota_cmd_h2d_negotiation_result(struct svc_prot_context *ctx, u16_t param_len)
{
	u8_t negotiation_result;
	int rlen;
...
int ota_cmd_require_image_data(struct svc_prot_context *ctx, u32_t offset, int len, u8_t *buf)
{
	u8_t read_mask, *send_buf;
	int err, send_len;
...
int ota_cmd_h2d_send_image_data(struct svc_prot_context *ctx, u16_t param_len, int with_crc)
{
	int err, seg_len;
	u8_t psn;
	u32_t crc, crc_orig;
	...
	/* read psn */
	err = svc_prot_get_rx_data(ctx, &psn, sizeof(u8_t));
	if (err) {
		SYS_LOG_ERR("read error, err %d", err);
		return -EIO;
	}
	param_len -= sizeof(u8_t);

	SYS_LOG_DBG("last_psn %d, cur psn %d\n", ctx->last_psn, psn);

	if (psn != (u8_t)(ctx->last_psn + 1)) {
	...
	if (with_crc) {
		/* read crc field */
		err = svc_prot_get_rx_data(ctx, (u8_t *)&crc_orig, sizeof(u32_t));
		if (err) {
			SYS_LOG_ERR("read error, err %d", err);
			return -EIO;
		}

		param_len -= sizeof(u32_t);
	}
...
int ota_cmd_h2d_send_image_data_with_crc(struct svc_prot_context *ctx, u16_t param_len)
{
	return ota_cmd_h2d_send_image_data(ctx, param_len, 1);
}

int ota_cmd_h2d_send_image_data_no_crc(struct svc_prot_context *ctx, u16_t param_len)
{
...
int process_command(struct svc_prot_context *ctx, u32_t *processed_cmd)
{
	struct svc_prot_head head;
	...
	err = svc_prot_get_rx_data(ctx, (u8_t *)&head, sizeof(struct svc_prot_head));
	if (err) {
					...
					struct ota_backend_bt_init_param *param)
{
	...
	init_param.spp_uuid = (u8_t *)param->spp_uuid;
