...
static int _btsrv_hfp_ag_cmd_cb(struct bt_conn *conn, u8_t at_type, void *param)
{
...
static int _btsrv_hfp_ag_update_indicator(int index, u8_t value)
{
	return hostif_bt_hfp_ag_update_indicator(index,value);
}

static int _btsrv_hfp_ag_send_event( char *event, u16_t len)
{
	struct bt_conn* conn = btsrv_rdm_hfp_get_actived();
	if(conn){
