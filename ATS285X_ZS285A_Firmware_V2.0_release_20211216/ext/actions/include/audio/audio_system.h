...
struct audio_record_t {
	...
	s16_t adc_gain;
	s16_t input_left_gain;
	s16_t input_right_gain;
	u16_t volume;
	u16_t pcm_buff_size;
	u8_t *pcm_buff;
	/** audio hal handle*/
...
/**
 * @brief get audio system output resample
 *
 * @details This routine provides to get CONFIG_OUTPUT_RESAMPLE,
 *
 * @return value of CONFIG_OUTPUT_RESAMPLE
 */
 int audio_system_is_support_resample(void);

/**
 * @brief get audio system support resample
 *
...
/**
 * @brief set audio system audio path delay
 *
 * @details This routine provides to set audio system audio path delay
 *
 * @param delay delay time of audio path, (unit: 1/10 milliseconds)
 *
 * @return  0: success ,others failed
 */
int audio_system_set_audio_delay(int delay);

/**
 * @brief get audio system audio path delay
 *
 * @details This routine provides to audio path delay
 *
 * @return  delay:  (unit: 1/10 milliseconds)
 */
int audio_system_get_audio_delay(void);
...
void audio_aps_monitor_init(int stream_type, int ext_stream_type, void *tws_observer, struct audio_track_t *audio_track);
...
void audio_aps_tws_notify_sample_rate_change(u32_t sample_rate);
