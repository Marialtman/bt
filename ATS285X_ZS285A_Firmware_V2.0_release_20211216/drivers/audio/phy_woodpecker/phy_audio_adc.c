...
static int adc_gain_config(audio_lch_input_e left_input, audio_rch_input_e right_input, s16_t left_gain, s16_t right_gain)
{
	...
	if (left_input_ch == right_input_ch) {
		...
		if ((INPUT_LCH_DISABLE != left_input) && (INPUT_RCH_DISABLE != right_input)) {
			/* Always guarantee the channel with smaller gain */
			if (left_gain < right_gain) {
				...
				adc_dig_vol_set(dig_gain, ADC_GAIN_INVALID);
				inputx_analog_gain_set(left_input_ch, input_res, fd_res);
				u16_t delta_gain = right_gain - left_gain;
				u8_t input_res_bak = input_res;
				input_res = delta_gain / 60;
				if (input_res > 3)
					input_res = 3;

				if (input_res == 3) {
					dig_gain = (input_res == input_res_bak)? 0 : ((input_res - input_res_bak) * 60 + 80);
				} else {
					dig_gain = (input_res == input_res_bak)? 0 : ((input_res - input_res_bak) * 60);
				}

				dig_gain = (delta_gain - dig_gain);
				if (dig_gain >= 0)
					adc_dig_vol_set(ADC_GAIN_INVALID, ((dig_gain + 34) / 35) - 1);
				...
				if (adc_aux_amic_gain_translate(right_gain, &input_res, &fd_res, &dig_gain)) {
					SYS_LOG_DBG("failed to translate amic_aux right gain %d", left_gain);
					return -EFAULT;
				}
				adc_dig_vol_set(ADC_GAIN_INVALID, dig_gain);
				inputx_analog_gain_set(right_input_ch, input_res, fd_res);
				u16_t delta_gain = left_gain - right_gain;
				u8_t input_res_bak = input_res;
				input_res = delta_gain / 60;
				if (input_res > 3)
					input_res = 3;
				...
				dig_gain = (delta_gain - dig_gain);
				if (dig_gain >= 0)
					adc_dig_vol_set(((dig_gain + 34) / 35) - 1, ADC_GAIN_INVALID);
